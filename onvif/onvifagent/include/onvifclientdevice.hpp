...
inline int OnvifClientDevice::GetCapabilities(_tds__GetCapabilitiesResponse &cap)
{
	...
	{
        std::cout<<"Can not get Capabilities"<<std::endl;
        return SOAP_ERR;
	}
	...
	if (cap.Capabilities->Media != NULL)
	{
        ...
        printf("media xaddr: %s\n", cap.Capabilities->Media->XAddr.c_str());
	...
	if (cap.Capabilities->PTZ != NULL)
	{
        ...
        printf("PTZ xaddr: %s\n", cap.Capabilities->PTZ->XAddr.c_str());
	}
	...
	if (cap.Capabilities->Events != NULL)
	{
        ...
        printf("EVENTS xaddr: %s\n", cap.Capabilities->Events->XAddr.c_str());
	}
	...
	if (cap.Capabilities->Imaging != NULL)
	{
        ...
        printf("Imaging xaddr: %s\n", cap.Capabilities->Imaging->XAddr.c_str());
	}
